...
void Content_to_server(char header[200], char data[100])
{
    char content[100] = {0};
    char Dau_hai_cham[2] = ":";
    strcat(content, header);
    strcat(content, Dau_hai_cham);
    strcat(content, data);
    printf("Data is :%s", content);
    write(fd_server, content, 100);
}
...
void Send_data_to_Server(char header[200])
{
    ...
    if(byte_send < BYTE_NUMBER) {
        ...
        printf("data send is %s:%s and bytesend : %ld\n", header, data, (strlen(header) + strlen(data) + 1));
        // write(fd_server, header, strlen(header));
        // write(fd_server, ":", strlen(":"));
        // write(fd_server, data, strlen(data));
        // sleep(1);
        Content_to_server(header, data);
    }else{
        printf("data send %s: %s and bytesend : %ld\n", header, data, (strlen(header) + strlen(data) + 1));
        write(fd_server, header, strlen(header));
        write(fd_server, ":", strlen(":"));
        write(fd_server, data, (BYTE_NUMBER - strlen(header) - 1));
    }
}
